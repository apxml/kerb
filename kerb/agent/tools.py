...
import asyncio
import inspect
from typing import List, Dict, Any, Optional, Callable, Union, Tuple, get_type_hints, TYPE_CHECKING
...
class ToolRegistry:
    ...
    def register(
        ...
        if tool.name in self._tools and not overwrite:
            raise ValueError(f"Tool '{tool.name}' already registered. Use overwrite=True to replace.")
...
def parallel_execute_tools(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for i, (tool, args, kwargs) in enumerate(tool_calls):
            future = executor.submit(execute_tool, tool, *args, registry=registry, **kwargs)
...
def format_tool_result(result: ToolResult, format: Union['ToolResultFormat', str] = "text") -> str:
...
def discover_tools_from_class(cls) -> List[Tool]:
    ...
    for name, obj in inspect.getmembers(instance):
        if (inspect.ismethod(obj) and not name.startswith('_') and
            name not in ['__init__', '__new__']):
            tool = create_tool(
