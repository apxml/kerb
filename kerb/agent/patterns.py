...
class ReActAgent(Agent):
    ...
    def run(self, goal: str, context: Dict[str, Any] = None) -> AgentResult:
        ...
        state = AgentState(goal=goal, max_iterations=self.max_iterations, context=context or {})
        ...
        if state.steps:
            ...
            for step in reversed(state.steps):
                if 'final answer' in step.thought.lower():
                    ...
                    match = re.search(r'Final Answer:\s*(.+)', step.thought, re.IGNORECASE | re.DOTALL)
                    if match:
    ...
    def _create_react_prompt(self, state: AgentState) -> str:
        ...
        prompt += "What should you think about next? Respond with: Thought: [your reasoning]"
        return prompt
...
class ChainOfThoughtAgent(Agent):
    ...
    def _create_cot_prompt(self, goal: str, context: Dict[str, Any] = None) -> str:
        ...
        prompt += "Please think through this carefully, showing your reasoning at each step."
        return prompt
    ...
    def _parse_reasoning_steps(self, response: str) -> List[str]:
        ...
        for line in lines:
            ...
            if re.match(r'^(Step \d+|First|Second|Third|Next|Finally|Therefore)', line, re.IGNORECASE):
                if current_step:
...
class TreeOfThoughtAgent(Agent):
    ...
    def _explore_paths(self, goal: str, context: Dict[str, Any] = None) -> List[Dict[str, Any]]:
    ...
    def _explore_path(
        ...
        next_thoughts = self._generate_thoughts(
            goal,
            context,
            previous_thoughts=current_path,
            count=self.breadth
        )
    ...
    def _generate_thoughts(
        ...
        prompt += f"Generate {count} different next reasoning steps to solve this problem."
