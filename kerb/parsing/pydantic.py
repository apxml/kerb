...
def parse_to_pydantic(text: str, model_class: Type, mode: ParseMode = ParseMode.LENIENT) -> ParseResult:
...
def pydantic_to_schema(model_class: Type) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ImportError("pydantic package not installed. Install with: pip install pydantic")
...
def validate_pydantic(data: Dict[str, Any], model_class: Type) -> ValidationResult:
    ...
    try:
        ...
        return ValidationResult(
            valid=False,
            errors=["pydantic package not installed. Install with: pip install pydantic"],
            data=data
        )
    except Exception as e:
        errors = []
...
def pydantic_to_function(model_class: Type, name: Optional[str] = None, description: Optional[str] = None) -> Dict[str, Any]:
