...
from .json import extract_json, extract_json_array, extract_json_object, fix_json
...
def validate_output(
    ...
    if schema and data and not errors:
        schema_result = validate_json_schema(data if not hasattr(data, 'model_dump') else data.model_dump(), schema)
...
def retry_parse_with_fixes(
    text: str,
    parser_func: Callable[[str], ParseResult],
    max_attempts: int = 3
) -> ParseResult:
