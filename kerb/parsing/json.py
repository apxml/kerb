...
def extract_json(text: str, mode: ParseMode = ParseMode.LENIENT) -> ParseResult:
    ...
    if matches:
        for match in matches:
            try:
                ...
                return ParseResult(success=True, data=data, original=original, warnings=warnings)
            except json.JSONDecodeError:
                continue
    ...
    if mode == ParseMode.STRICT:
        return ParseResult(success=False, error="No valid JSON found in text", original=original)
