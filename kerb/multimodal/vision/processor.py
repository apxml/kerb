...
def extract_dominant_colors(file_path: str, num_colors: int = 5) -> List[Tuple[int, int, int]]:
...
def analyze_image_with_vision_model(
    ...
    if model_str.startswith('gpt-4'):
        return _analyze_openai_vision(image_path, prompt, model_str, api_key, max_tokens)
    elif model_str.startswith('claude-3'):
        return _analyze_anthropic_vision(image_path, prompt, model_str, api_key, max_tokens)
    elif model_str.startswith('gemini'):
        return _analyze_google_vision(image_path, prompt, model_str, api_key, max_tokens)
    else:
...
def embed_multimodal(
    ...
    content_type: str,
    model: Union[str, EmbeddingModelMultimodal] = EmbeddingModelMultimodal.CLIP_VIT_B_32,
