...
from ..types import AudioFormat, AudioInfo, TranscriptionModel, TranscriptionResult
...
def get_audio_info(file_path: str) -> AudioInfo:
    ...
    try:
        ...
        raise ImportError("mutagen is required for audio processing. Install with: pip install mutagen")
...
def transcribe_audio(
    ...
    if model_str == "whisper-1" or model_str == "openai_whisper_1":
        return _transcribe_openai_whisper(file_path, api_key, language, return_timestamps)
    
    # Local Whisper models
    else:
        return _transcribe_local_whisper(file_path, model_str, language, return_timestamps)
...
def _transcribe_openai_whisper(
    ...
    try:
        ...
        raise ImportError("openai is required for OpenAI Whisper. Install with: pip install openai")
...
def _transcribe_local_whisper(
    ...
    try:
        ...
        raise ImportError("whisper is required for local transcription. Install with: pip install openai-whisper")
...
async def transcribe_audio_async(
    ...
    if model_str == "whisper-1" or model_str == "openai_whisper_1":
        return await _transcribe_openai_whisper_async(
            file_path, api_key, language, return_timestamps, max_size_mb, max_duration_minutes
        )
