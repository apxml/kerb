...
from typing import List, Dict, Any, Union, Iterator, Optional, Callable
...
def write_jsonl(
    ...
    if compress and not any(filepath.endswith(ext) for ext in ['.gz', '.bz2', '.xz', '.lzma']):
...
def merge_jsonl(
    ...
    """
    if compress_output and not any(output_file.endswith(ext) for ext in ['.gz', '.bz2', '.xz']):
...
def filter_jsonl(
    ...
    with open_func_out(output_file, mode_write, encoding='utf-8') as outf:
        for batch in stream_jsonl(input_file, batch_size=batch_size, filter_fn=filter_fn):
            for item in batch:
...
def transform_jsonl(
    ...
    with open_func_out(output_file, mode_write, encoding='utf-8') as outf:
        if parallel:
            ...
            for batch in stream_jsonl(input_file, batch_size=batch_size, transform_fn=transform_fn):
                for item in batch:
                    outf.write(json.dumps(item, ensure_ascii=False) + '\n')
...
def get_jsonl_stats(filepath: str, sample_size: int = 1000) -> Dict[str, Any]:
    ...
    stats = {
        'total_lines': 0,
        'total_bytes': 0,
        'compressed': any(filepath.endswith(ext) for ext in ['.gz', '.bz2', '.xz', '.lzma']),
        'keys': set(),
        'sample_items': []
    }
    ...
    stats['avg_bytes_per_line'] = stats['total_bytes'] / stats['total_lines'] if stats['total_lines'] > 0 else 0
    
    return stats


def compress_jsonl(input_file: str, output_file: Optional[str] = None, compression_type: str = 'gz') -> str:
