...
from .enums import LLMProvider, MODEL_PRICING, ModelName
...
class RateLimiter:
    ...
    def __init__(self, requests_per_minute: int = 60, tokens_per_minute: Optional[int] = None):
...
class ResponseCache:
    ...
    def get(self, messages: List[Message], config: GenerationConfig) -> Optional[GenerationResponse]:
    ...
    def set(self, messages: List[Message], config: GenerationConfig, response: GenerationResponse) -> None:
        """Cache a response."""
...
def parse_json_response(response: Union[GenerationResponse, str]) -> Dict[str, Any]:
    ...
    if result.success:
        ...
        raise ValueError(f"Failed to parse JSON from response: {result.error}\nContent: {content[:200]}")
...
def batch_generate(
    ...
    **kwargs
    ...
    messages_list = [[Message(role=MessageRole.USER, content=prompt)] for prompt in prompts]
