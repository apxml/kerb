...
def assess_coherence(text: str) -> EvaluationResult:
    ...
    has_transitions = any(
        any(word in sent.lower() for word in transition_words)
    ...
    sent_length_variance = statistics.variance(len(s.split()) for s in sentences) if len(sentences) > 1 else 0
    ...
    pronoun_usage = sum(
        1 for sent in sentences[1:]  # Skip first sentence
...
def assess_fluency(text: str) -> EvaluationResult:
    ...
    if len(words) > 0:
        ...
        significant_words = {w: c for w, c in word_freq.items() if w not in common_words and len(w) > 3}
...
def detect_hallucination(
    ...
    for sent in output_sentences:
        ...
        support_ratio = words_in_context / len(significant_words) if significant_words else 1.0
        
        if support_ratio < threshold:
            unsupported_sentences += 1
            details.append(f"Unsupported: '{sent[:50]}...' (support: {support_ratio:.2f})")
    
    hallucination_score = unsupported_sentences / total_sentences if total_sentences > 0 else 0.0
...
def assess_faithfulness(
    output: str,
    source: str,
    method: Union['FaithfulnessMethod', str] = "entailment"
) -> EvaluationResult:
...
def assess_answer_relevance(
    ...
    question_stopwords = {"what", "when", "where", "why", "how", "who", "which", "is", "are", "the", "a", "an"}
