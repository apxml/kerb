...
INJECTION_PATTERNS = [
    ...
    (r'ignore\s+(?:(?:all|any)\s+)?(?:previous|above|prior)\s+(?:instructions?|commands?|prompts?|directions?)', 3.0),
    (r'disregard\s+(?:(?:all|any)\s+)?(?:previous|above|prior)\s+(?:instructions?|commands?|prompts?)', 3.0),
    (r'forget\s+(?:everything|all|previous|above)(?:\s+(?:instructions?|commands?|prompts?))?', 2.5),
...
JAILBREAK_PATTERNS = [
    ...
    (r'without\s+(?:any\s+)?(?:restrictions?|limitations?|filters?|rules?|ethics?)', 3.0),
...
VIOLENCE_PATTERNS = [
    # Direct threats
    (r'\b(?:kill|murder|assassinate|execute)\s+(?:you|someone|them|him|her|everyone)\b', 4.0),
    ...
    # Physical violence
    (r'\b(?:beat|attack|assault|brutalize)\s+(?:the\s+)?(?:shit|crap|hell)\s+out\s+of\b', 3.0),
    (r'\b(?:punch|kick|hit|strike|slap|choke)\s+(?:someone|you|them|him|her|your)\b', 2.5),
    
    # General violence
    (r'\b(?:violent|violence|brutal|savage|bloody)\s+(?:act|attack|scene|murder)\b', 2.0),
...
HATE_SPEECH_PATTERNS = [
    ...
    (r'\b(?:hate|despise|detest)\s+(?:all|those|these)\s+\w+\s+(?:people|folks|guys)\b', 4.0),
    (r'\b(?:all|those)\s+(?:those|these)?\s*\w+\s+(?:are|should\s+be)\s+(?:inferior|worthless|garbage|scum)\b', 4.0),
    ...
    # Superiority claims
    (r'\b(?:inferior|superior)\s+(?:race|gender|ethnicity|religion|group|people)\b', 3.5),
