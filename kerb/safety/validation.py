...
from .pii import detect_pii, redact_pii, detect_url
...
def detect_code_injection(text: str) -> SafetyResult:
    ...
    matches = sum(1 for pattern in dangerous_patterns if re.search(pattern, text, re.IGNORECASE))
    score = max(0.0, 1.0 - (matches / len(dangerous_patterns)))
    safe = matches == 0
    
    reason = f"Detected potential code injection ({matches} patterns)" if not safe else None
