...
def embed(
    ...
    **kwargs
    ...
    """
    from .providers.local import local_embed
    from .providers.openai import openai_embed
    from .providers.local import sentence_transformer_embed
...
def embed_batch(
    ...
    **kwargs
    ...
    from .providers.local import sentence_transformer_embed_batch
    ...
    if backend == ModelBackend.LOCAL:
        ...
        return openai_embed_batch(texts, model_name=model_str, api_key=api_key, batch_size=batch_size, **kwargs)
    else:  # SENTENCE_TRANSFORMERS
        return sentence_transformer_embed_batch(texts, model_name=model_str, batch_size=batch_size, **kwargs)
...
async def embed_async(
    ...
    **kwargs
    ...
    if backend == ModelBackend.OPENAI:
        return await openai_embed_async(text, model_name=model_str, api_key=api_key, **kwargs)
    else:
    ...
    if backend == ModelBackend.OPENAI:
        ...
        async def process_batch(batch_start, batch_texts):
            async with semaphore:
                ...
                return [(batch_start + i, item.embedding) 
