...
from .strategies import generate_prompt_key, generate_embedding_key
...
class LLMCache:
    ...
    def get_cached_prompt(
        ...
        **kwargs
        ...
        if cached is not None and isinstance(self.backend, MemoryCache):
            ...
            if entry:
                cost = entry.metadata.get("cost", self.cost_per_token * self.avg_tokens_per_request)
    ...
    def get_stats(self) -> CacheStats:
        ...
        if isinstance(self.backend, (MemoryCache, DiskCache)):
            ...
            combined.evictions = stats_dict["memory"].evictions + stats_dict["disk"].evictions
