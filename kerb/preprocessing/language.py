...
def detect_language(
    text: str,
    mode: LanguageDetectionMode = LanguageDetectionMode.FAST
) -> LanguageResult:
...
def get_supported_languages() -> List[str]:
    ...
    try:
        ...
        return heuristic_langs + [
            "af", "sq", "am", "bg", "bn", "ca", "hr", "et", "tl", "ka",
            "gu", "ht", "he", "id", "ga", "kn", "lv", "lt", "mk", "ml",
            "mr", "mn", "ne", "pa", "fa", "sk", "sl", "so", "sw", "ta",
            "te", "uk", "ur", "vi", "cy", "yi"
        ]
    except ImportError:
        return heuristic_langs
...
def _detect_language_heuristic(text: str, mode: LanguageDetectionMode) -> LanguageResult:
    ...
    return script_result if script_result.confidence > 0.3 else LanguageResult(
...
def _detect_by_script(text: str) -> LanguageResult:
    ...
    if char_counts['arabic'] / total_chars > 0.3:
        return LanguageResult(language="ar", confidence=min(0.95, char_counts['arabic'] / total_chars + 0.2))
    
    if char_counts['hebrew'] / total_chars > 0.3:
        return LanguageResult(language="he", confidence=min(0.95, char_counts['hebrew'] / total_chars + 0.2))
    
    if char_counts['cyrillic'] / total_chars > 0.3:
        # Could be Russian, Ukrainian, Bulgarian, etc.
        return LanguageResult(language="ru", confidence=min(0.85, char_counts['cyrillic'] / total_chars + 0.1))
    
    if char_counts['greek'] / total_chars > 0.3:
        return LanguageResult(language="el", confidence=min(0.95, char_counts['greek'] / total_chars + 0.2))
    
    if char_counts['devanagari'] / total_chars > 0.3:
        return LanguageResult(language="hi", confidence=min(0.90, char_counts['devanagari'] / total_chars + 0.15))
    
    if char_counts['thai'] / total_chars > 0.3:
        return LanguageResult(language="th", confidence=min(0.95, char_counts['thai'] / total_chars + 0.2))
    
    if char_counts['hangul'] / total_chars > 0.2:
        return LanguageResult(language="ko", confidence=min(0.95, char_counts['hangul'] / total_chars + 0.25))
    ...
    if japanese_chars / total_chars > 0.1:
        return LanguageResult(language="ja", confidence=min(0.95, (japanese_chars / total_chars) * 2 + 0.3))
    
    # Chinese if CJK chars without Japanese kana
    if char_counts['cjk'] / total_chars > 0.3 and japanese_chars == 0:
        return LanguageResult(language="zh", confidence=min(0.90, char_counts['cjk'] / total_chars + 0.15))
    
    # Latin script - need further analysis
    if char_counts['latin'] / total_chars > 0.5:
        return LanguageResult(language="en", confidence=0.4)  # Low confidence, needs further analysis
...
def _detect_latin_language(text_lower: str) -> LanguageResult:
    ...
    patterns = {
        'fr': (r'[àâæçéèêëîïôùûüÿœ]', ['le', 'la', 'les', 'de', 'et', 'est', 'un', 'une', 'dans', 'pour', 'que', 'qui', 'avec', 'ce', 'il', 'ne', 'pas', 'se', 'vous', 'sont']),
        'de': (r'[äöüß]', ['der', 'die', 'das', 'und', 'ist', 'ein', 'eine', 'nicht', 'mit', 'den', 'sich', 'auf', 'für', 'von', 'dem', 'zu', 'im', 'werden', 'auch', 'wie']),
        'es': (r'[áéíñóúü¿¡]', ['el', 'la', 'de', 'que', 'y', 'en', 'un', 'es', 'por', 'los', 'una', 'con', 'del', 'las', 'al', 'se', 'lo', 'como', 'más', 'pero']),
        'pt': (r'[ãõáàâéêíóôõúüç]', ['o', 'a', 'de', 'que', 'e', 'do', 'da', 'em', 'um', 'para', 'com', 'não', 'os', 'as', 'dos', 'uma', 'na', 'no', 'ao', 'ser']),
        'it': (r'[àèéìíîòóùú]', ['il', 'di', 'e', 'la', 'che', 'per', 'un', 'non', 'in', 'una', 'è', 'sono', 'del', 'le', 'da', 'si', 'con', 'dei', 'alla', 'anche']),
        'pl': (r'[ąćęłńóśźż]', ['się', 'na', 'jest', 'z', 'do', 'i', 'w', 'nie', 'to', 'co', 'o', 'za', 'od', 'po', 'dla', 'te', 'jak', 'ze', 'może', 'być']),
        'ro': (r'[ăâîșțşţ]', ['de', 'în', 'și', 'la', 'cu', 'pe', 'ca', 'pentru', 'este', 'un', 'o', 'ce', 'din', 'al', 'se', 'sunt', 'să', 'mai', 'sau', 'a']),
        'cs': (r'[áčďéěíňóřšťúůýž]', ['je', 'se', 'na', 'v', 'že', 'a', 's', 'z', 'o', 'k', 'do', 'i', 'to', 'jako', 'pro', 'jsou', 'si', 'od', 'po', 'ale']),
        'tr': (r'[çğıİöşü]', ['ve', 'bir', 'bu', 'için', 'ile', 'olan', 'da', 'de', 'var', 'mi', 'ne', 'olarak', 'daha', 'gibi', 'en', 'her', 'kadar', 'çok', 'o', 'ya']),
        'sv': (r'[åäö]', ['och', 'att', 'i', 'en', 'är', 'det', 'som', 'på', 'för', 'med', 'till', 'av', 'om', 'har', 'den', 'inte', 'var', 'ett', 'han', 'men']),
        'no': (r'[åæø]', ['og', 'i', 'det', 'er', 'en', 'til', 'på', 'som', 'for', 'med', 'ikke', 'av', 'han', 'har', 'den', 'var', 'om', 'så', 'hun', 'kan']),
        'da': (r'[åæø]', ['og', 'i', 'det', 'er', 'at', 'en', 'til', 'på', 'som', 'for', 'med', 'ikke', 'den', 'af', 'har', 'de', 'han', 'var', 'jeg', 'om']),
        'nl': (r'[áéíóúàèëïöü]', ['de', 'het', 'en', 'van', 'een', 'in', 'is', 'dat', 'op', 'te', 'voor', 'met', 'die', 'aan', 'niet', 'als', 'zijn', 'wordt', 'ook', 'om']),
        'fi': (r'[äö]', ['ja', 'on', 'ei', 'että', 'se', 'oli', 'kun', 'hän', 'mutta', 'tai', 'olla', 'ovat', 'voi', 'kuin', 'niin', 'jos', 'siitä', 'olen', 'ne', 'mitä']),
        'hu': (r'[áéíóöőúüű]', ['a', 'az', 'és', 'van', 'egy', 'hogy', 'nem', 'meg', 'de', 'ha', 'volt', 'is', 'ki', 'csak', 'mint', 'már', 'el', 'be', 'még', 'le']),
    }
    ...
    if word_count > 0:
        en_common = ['the', 'is', 'are', 'of', 'and', 'to', 'in', 'a', 'that', 'it', 'for', 'as', 'with', 'was', 'be', 'on', 'at', 'by', 'this', 'have']
        ...
        ascii_ratio = sum(1 for c in text_lower if ord(c) < 128) / max(1, len(text_lower))
