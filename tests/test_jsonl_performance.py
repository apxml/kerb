...
class TestCompression:
    ...
    def test_compression_types(self):
        ...
        for comp_type, ext in [('gz', '.gz'), ('bz2', '.bz2'), ('xz', '.xz')]:
            ...
            try:
                jsonl.write_jsonl(data, filepath, compress=True, compression_type=comp_type)
                read_data = jsonl.read_jsonl(filepath)
...
class TestStreaming:
    ...
    def test_stream_with_filter(self):
        ...
        try:
            ...
            filtered = []
            for batch in jsonl.stream_jsonl(filepath, filter_fn=lambda x: x["value"] == 0):
    ...
    def test_stream_with_transform(self):
        ...
        try:
            ...
            for batch in jsonl.stream_jsonl(
                filepath,
                transform_fn=lambda x: {"value": x["value"], "squared": x["value"]**2}
            ):
...
class TestParallelReading:
    ...
    def test_parallel_read_jsonl(self):
        ...
        try:
            # Read in parallel
            read_data = jsonl.parallel_read_jsonl(filepath, num_workers=2, chunk_size=100)
            assert len(read_data) == len(data)
            assert read_data[0] == data[0]
        finally:
            Path(filepath).unlink(missing_ok=True)
...
class TestMerging:
    ...
    def test_merge_jsonl(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.jsonl', delete=False) as f1, \
             ...
             tempfile.NamedTemporaryFile(mode='w', suffix='.jsonl', delete=False) as f3, \
             tempfile.NamedTemporaryFile(suffix='.jsonl', delete=False) as fout:
