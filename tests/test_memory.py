...
from kerb.memory import (
    Entity, ConversationSummary, ConversationBuffer,
    create_sliding_window, create_token_limited_window, create_alternating_window,
    create_progressive_summary, summarize_conversation, create_hierarchical_summary,
    save_conversation, load_conversation,
)
from kerb.memory.entities import (
    extract_entities, track_entity_mentions, extract_entity_relationships, merge_entities,
)
from kerb.memory.utils import (
    format_messages, filter_messages, merge_conversations, prune_buffer,
)
from kerb.memory.patterns import (
    create_semantic_memory, create_episodic_memory, get_relevant_memory
)
...
def test_message_serialization():
    ...
    restored_role = restored.role.value if hasattr(restored.role, 'value') else restored.role
