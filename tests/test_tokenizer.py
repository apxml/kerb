...
def test_count_tokens_for_messages_basic():
    ...
    min_expected = (len(messages[0]["content"]) // 4) + (len(messages[1]["content"]) // 4) + 11
...
def test_truncate_to_token_limit_no_truncation():
    ...
    result = truncate_to_token_limit(text, max_tokens=100, tokenizer=Tokenizer.CL100K_BASE)
    assert result == text
...
def test_truncate_with_very_small_limit():
    ...
    result = truncate_to_token_limit(text, max_tokens=1, tokenizer=Tokenizer.CL100K_BASE)
...
try:
    ...
    def test_truncate_with_tiktoken():
        ...
        result = truncate_to_token_limit(text, max_tokens=10, tokenizer=Tokenizer.CL100K_BASE)
