...
from kerb.embedding.embedder import _get_model_backend, ModelBackend
...
class TestEmbeddingModelEnum:
    ...
    def test_enum_values(self):
        ...
        assert EmbeddingModel.PARAPHRASE_MPNET_BASE_V2.value == "paraphrase-mpnet-base-v2"
...
class TestBackendDetection:
    ...
    def test_openai_backend_detection_enum(self):
        ...
        assert _get_model_backend(EmbeddingModel.TEXT_EMBEDDING_3_SMALL) == ModelBackend.OPENAI
        assert _get_model_backend(EmbeddingModel.TEXT_EMBEDDING_3_LARGE) == ModelBackend.OPENAI
        assert _get_model_backend(EmbeddingModel.TEXT_EMBEDDING_ADA_002) == ModelBackend.OPENAI
    
    def test_sentence_transformers_backend_detection(self):
        """Test Sentence Transformers backend (default)."""
        assert _get_model_backend("all-MiniLM-L6-v2") == ModelBackend.SENTENCE_TRANSFORMERS
        assert _get_model_backend("all-mpnet-base-v2") == ModelBackend.SENTENCE_TRANSFORMERS
        assert _get_model_backend("custom-model-name") == ModelBackend.SENTENCE_TRANSFORMERS
        assert _get_model_backend("bert-base-uncased") == ModelBackend.SENTENCE_TRANSFORMERS
        assert _get_model_backend("path/to/custom/model") == ModelBackend.SENTENCE_TRANSFORMERS
        
        # Test with enum
        assert _get_model_backend(EmbeddingModel.ALL_MINILM_L6_V2) == ModelBackend.SENTENCE_TRANSFORMERS
        assert _get_model_backend(EmbeddingModel.ALL_MPNET_BASE_V2) == ModelBackend.SENTENCE_TRANSFORMERS
