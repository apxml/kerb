...
def test_basic_functionality():
    ...
    data = [
        ...
        },
        {
            "messages": [
                ...
                {"role": "assistant", "content": "JavaScript is a programming language for web development."}
            ]
        },
        {
            "messages": [
    ...
    assert result.valid_examples == 3, f"Expected 3 valid examples, got {result.valid_examples}"
    ...
    large_dataset = prepare_dataset(large_data, format=DatasetFormat.CHAT, validate=False, deduplicate=False)
    ...
    assert total_split == len(large_dataset), f"Split sizes don't add up: {total_split} != {len(large_dataset)}"
    print(f"   ✓ Split {len(large_dataset)} examples -> Train: {len(train)}, Val: {len(val)}, Test: {len(test)}")
