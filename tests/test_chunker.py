...
def test_simple_chunker_documentation():
    ...
    assert len(chunks) >= 2
    assert all(len(chunk) <= 150 for chunk in chunks[:-1])  # All but last should be max size
...
def test_semantic_chunker_paragraph_structure():
    ...
    assert len(chunks) == 4
    assert "Large language models are trained on massive datasets. They learn to predict the next token in a sequence." == chunks[0]
    assert "This training approach is called autoregressive modeling. The models develop emergent capabilities at scale." == chunks[1]
